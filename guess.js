/* =========================================================
   WOOO — Page de vote (guess) — version refonte
   ---------------------------------------------------------
   Flux :
   1) Countdown 3-2-1-GO en intro (3 sec)
   2) Affiche la chanson en cours (vinyle + titre + artiste + audio)
   3) Le joueur drag-drop OU clique sur un tag pour l'attribuer au vinyle
   4) Suivante automatique (ou bouton)
   5) À la dernière chanson, on redirige vers recap.html

   Drag-and-drop :
   - Souris (desktop) : mousedown / mousemove / mouseup
   - Tactile (mobile) : touchstart / touchmove / touchend
   - Fallback clic : si le joueur clique sans drag (mouse/touch < 5px),
     on ouvre une mini-popup pour choisir la chanson cible
   ========================================================= */
...
(function () {
  ...
  async function loadAndStart() {
    ...
    try {
      ...
      if (allPicks.length === 0) {
        ...
        window.location.href = 'recap.html?theme=' + themeIndex;
        return;
      }
  ...
  function playAudio() {
    ...
    audio.play().then(() => {
      ...
      console.info('[Wooo] Autoplay bloqué :', err);
    });
  }
  ...
  // ======= INIT =======
  // On stocke les votes pour le passage à recap.html
